...
3. [Configure your preferred MCP client](#configuration).
  Jupyter MCP Server supports both `stdio` and `streamable HTTP` transport.
...
To start the server, you can either use `python` or `docker`.

The server will listen on port `4040`, you can access it via the URL `http://localhost:4040`.

#### Using Python

Clone the repository and use `pip install -e .` or install the `jupyter-mcp-server package` from PyPI with `pip install jupyter-mcp-server`.
