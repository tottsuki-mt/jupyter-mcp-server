...
2. `append_markdown_cell`
    - Append at the end of the notebook a markdown cell with the provided content.
    - Input:
      - `cell_source`(string): Markdown content.
...
7. `read_cell`
    ...
    - Returns: Dictionary with cell index, type, content, and outputs (for code cells).

8. `read_all_cells`
    - Read all cells from the notebook.
    - Returns:  List of cell information including index, type, content,
